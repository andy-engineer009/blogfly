...
export default function EditPostPage() {
  ...
  const blogPost = postId ? getBlogPostById(postId) : null;
  
  const [previewImage, setPreviewImage] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [existingImage, setExistingImage] = useState<string | null>(blogPost?.image || null);
  ...
  const formik = useFormik<{ title: string; description: string; category: string; author: string; image?: File | null }>({
    initialValues: {
      ...
      description: blogPost?.content.join("\n\n") || "",
    ...
    onSubmit: async (values, helpers) => {
      ...
      // Here you would save the data to your API
      console.log("Saving post:", values);
      helpers.setSubmitting(false);
    },
  });

  // Update existing image when blog post loads
  useEffect(() => {
    if (blogPost?.image) {
  ...
  const handleImageChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setFileName(null);
      setExistingImage(blogPost?.image || null); // Show existing image again if file is cleared
    }
  };
       ...
       <header className="flex flex-col gap-4 rounded-2xl border border-[var(--border-color)] bg-gradient-to-br from-[var(--surface)] via-[var(--surface)] to-[var(--card)] p-6 shadow-lg sm:flex-row sm:items-center sm:justify-between sm:p-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold text-[var(--foreground)] sm:text-3xl">Edit Blog</h1>
          </div>
        </div>
      </header>
        ...
        <form onSubmit={formik.handleSubmit} className="flex flex-col gap-6">
          ...
          {existingImage && !previewImage && (
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-[var(--foreground)]">Current Image</label>
              <div className="rounded-[1rem] border border-[var(--border-color)] bg-white p-4 shadow-[0_10px_20px_rgba(15,23,42,0.1)]">
                <div className="relative h-64 w-full overflow-hidden rounded-[0.75rem]">
                  <Image
                    src={existingImage}
                    alt={formik.values.title || "Blog image"}
                    fill
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="Whatever story you want to capture"
            />
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-3 rounded-[1rem] border border-dashed border-[var(--border-color)] bg-white/80 p-4 text-sm text-[var(--muted)] shadow-[0_10px_25px_rgba(15,23,42,0.08)]">
              <p className="text-[0.85rem]">
                Drag and drop or click to select the image. Recommended
              </p>
